...
export default {
    getList: (resource, params) => {
        ...
        if(q.query.type) {
        q.query.type={$regex: '.*' + q.query.type + '.*', $options: 'i'}
        }
        
        if(q.query.supType) {
        q.query.supType={$regex: '.*' + q.query.supType + '.*', $options: 'i'}
        }
        ...
        if(q.query.referenceEntityName) {
        q.query.referenceEntityName={$regex: '.*' + q.query.referenceEntityName + '.*', $options: 'i'}
        }
        
        if(q.query.referenceTitleFromRecord) {
        q.query.referenceTitleFromRecord={$regex: '.*' + q.query.referenceTitleFromRecord + '.*', $options: 'i'}
        }
        
        if(q.query.referenceTitles) {
			q.query.referenceTitles={$in: q.query.referenceTitles}
		}
        
        if(q.query.entity) {
        q.query.entity={$regex: '.*' + q.query.entity + '.*', $options: 'i'}
        }
